...
namespace WorkTrackingSystem.Areas.HRManager.Controllers
{
    ...
    public class DashboardController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            ViewBag.TotalCompletedJobs = _context.Scores.Count(j => j.Status == 1 && j.IsActive == true && j.IsDelete == false);
            ViewBag.TotalEvaluated = _context.Baselineassessments.Count(b => b.Evaluate == true && b.IsActive == true && b.IsDelete == false);

            // Trạng thái công việc
            ViewBag.JobStatusOntime = _context.Scores.Count(j => j.Status == 1);
            ViewBag.JobStatusOverdue = _context.Scores.Count(j => j.Status == 2);
            ViewBag.JobStatusLate = _context.Scores.Count(j => j.Status == 3);
            ViewBag.JobStatusProcessing = _context.Scores.Count(j => j.Status == 4);
